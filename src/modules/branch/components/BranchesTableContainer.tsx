...
const BranchesTableContainer = ({ tableHeaders, data, noDataMessage, setRefetchData }: any) => {
        ...
        <div className={`table_container`}>
                    ...
                    <>
                        {data?.map((item: any, index: number) => (
                            <div className="item has_logo" key={index}>
                                <div className="column capitalize">{item?.city || "-"}</div>
                                <div className="column capitalize">{item?.area || "-"}</div>
                                <div className="column ">{item?.deliveryPrice || "-"}</div>
                                <div className="column ">{item?.estimatedDeliveryTime || "-"}</div>
                                <div className="column status_col capitalize"><div className={`${item?.published ? "active" : ""}`}>{`${item?.published ? "active" : "inactive"}`}</div></div>
                                <div className="column actions actions_col">
                                    <div className="action_btn edit" onClick={(e) => { e.stopPropagation(); navigate(`/branches/create-branch/${item._id}`) }}>
                                    ...
                                    <div className="action_btn delete" onClick={(e) => { e.stopPropagation(); setDeleteModalOpen(item._id) }}>
            ...
            {deleteModalOpen && (
                <ModalContainer small>
                    <DeleteModal id={deleteModalOpen} setModal={setDeleteModalOpen} setRefetchData={setRefetchData} route={"/restaurant"} successMsg={"Restaurant deleted successfully"}
                        warningMsg={"Are you sure you want to delete this branch?"} />
                </ModalContainer>
            )}
        </div>
    );
...
export default BranchesTableContainer;